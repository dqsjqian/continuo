...
2. ~~QUIC/H3 over 真实 UDP~~：**已达成**——`quic::Connection` / `http3::Connection`（显式 pump 模型）在 kernel UDP 上端到端跑通：QUIC 全链路（握手+200KB 双向流+流控+关闭，0.4s）、HTTP/3 全链路（握手+POST/响应头+20KB 回显+流控+关闭）。曾经的"ACK 死锁"实为测试互等逻辑 bug，而非 ngtcp2 问题。
