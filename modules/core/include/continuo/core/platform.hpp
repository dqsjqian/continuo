...
// continuo/core/platform.hpp — the native handle, and what each platform can do.
//
// Continuo targets every platform its hosts ship on: macOS, Linux, Windows,
...
// then read). So Continuo's public I/O API is completion-shaped on every
...
#if defined(_WIN32)
    #define CONTINUO_PLATFORM_WINDOWS 1
    #define CONTINUO_IO_BACKEND_IOCP 1
#elif defined(__APPLE__)
    #include <TargetConditionals.h>
    #define CONTINUO_PLATFORM_APPLE 1
    #define CONTINUO_IO_BACKEND_KQUEUE 1
    #if TARGET_OS_IPHONE
        #define CONTINUO_PLATFORM_IOS 1
    #else
        #define CONTINUO_PLATFORM_MACOS 1
    #endif
#elif defined(__ANDROID__)
    #define CONTINUO_PLATFORM_ANDROID 1
    #define CONTINUO_IO_BACKEND_EPOLL 1
#elif defined(__linux__)
    #define CONTINUO_PLATFORM_LINUX 1
    #define CONTINUO_IO_BACKEND_EPOLL 1
#elif defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__NetBSD__)
    #define CONTINUO_PLATFORM_BSD 1
    #define CONTINUO_IO_BACKEND_KQUEUE 1
#else
    #error "continuo: unsupported platform — add a readiness or completion backend"
#endif
...
/// operations instead. Code inside `#if CONTINUO_HAS_READINESS_API` is by
/// definition not portable — the completion API is.
#if defined(CONTINUO_IO_BACKEND_KQUEUE) || defined(CONTINUO_IO_BACKEND_EPOLL)
    #define CONTINUO_HAS_READINESS_API 1
#else
    #define CONTINUO_HAS_READINESS_API 0
#endif
...
#if defined(CONTINUO_IO_BACKEND_IOCP)
    return IoBackend::iocp;
#elif defined(CONTINUO_IO_BACKEND_KQUEUE)
